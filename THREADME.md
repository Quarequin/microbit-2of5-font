...
- มีฟังก์ชัน `write` สำหรับแปลงตำแหน่งบิต 2 ตำแหน่งกลับเป็นตัวเลข (ใช้ binary search)
...
| ฟังก์ชัน | คำอธิบาย |
|---------|----------|
| `write(a, b)` | แปลงตำแหน่งบิต 2 ตำแหน่ง (0–4) เป็นตัวเลข 2of5 (คืนค่า -1 ถ้าไม่ถูกต้อง) |
| `showNumber(n, guard, transpose?, brightness?)` | แสดงตัวเลขเต็ม (สูงสุด 5 หรือ 6 หลัก) |
| `showSingleNumber(n, col, inv, transpose?, brightness?)` | แสดงตัวเลข 1 หลักที่ตำแหน่งคอลัมน์ที่กำหนด |
| `showDualNumber(a, b, align, transpose?, brightness?)` | แสดงตัวเลข 2 ค่าพร้อมกัน (ซ้าย / กลาง / ขวา) |
...
- **a, b** (ใน `write`) — ตำแหน่งบิต (0–4) ต้องไม่ซ้ำกัน
